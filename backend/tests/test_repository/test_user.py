...
async def test_create_one():
    admin_id = await user_repo.create_with_username("admin", id='1')
    assert admin_id == '1'

async def test_create_similar_2():
    admin_id_1 = await user_repo.create_with_username("admin", id='1')
    admin_id_2 = await user_repo.create_with_username("admin", id='1')
    assert admin_id_1 == '1'
    assert admin_id_2 == '1'

async def test_create_two():
    admin_id = await user_repo.create_with_username("admin", id='1')
    other_id = await user_repo.create_with_username("other", id='2')
    assert admin_id == '1'
    assert other_id == '2'

async def test_get(user_1_dto):
    admin_id = await user_repo.create_with_username("user_1", id='1')
...
async def test_get_two(user_1_dto, user_2_dto):
    admin_id = await user_repo.create_with_username("user_1", id='1')
    other_id = await user_repo.create_with_username("user_2", id='2')
    assert admin_id == '1'
    assert other_id == '2'
