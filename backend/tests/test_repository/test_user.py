...
async def test_create_similar_2():
    admin_id_1 = await user_repo.create_with_username("admin")
    admin_id_2 = await user_repo.create_with_username("admin")
    assert admin_id_1 == 1
    assert admin_id_2 == 2
...
async def test_get(user_1_dto):
    admin_id = await user_repo.create_with_username("user_1")
...
async def test_get_two(user_1_dto, user_2_dto):
    admin_id = await user_repo.create_with_username("user_1")
    other_id = await user_repo.create_with_username("user_2")
    assert admin_id == 1
    assert other_id == 2
